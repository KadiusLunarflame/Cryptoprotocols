//
// Created by kadius on 02.05.23.
//

#ifndef TESTPROJECT_STREEBOG_HPP
#define TESTPROJECT_STREEBOG_HPP
...
struct Streebog {
    ...
    void println(uint8_t* state) {
        for(int i{63}; i >= 0; --i) {
            std::cout << std::hex  << std::setfill('0') <<  std::setw(2) << (int)state[i];
        }
        std::cout << std::endl;
    }
    ...
    Streebog(size_t hash_length): hash_size(hash_length) {
        if (hash_length == 256) {
            uint64_t v{1};
            std::fill(h, h + 64, v);
        } else {
    ...
    void ring_addition(const uint8_t* a, uint8_t* reg) {
        int tmp{};

        for(int i{}; i < 64; ++i) {
            tmp >>= 8;
            tmp += (int)(reg[i]) + (int)(a[i]);
            reg[i] = (uint8_t)(tmp);
        }
...
//        println(state);
    ...
    void gen_key_i(size_t n, const uint8_t* src, uint8_t* dest) {
        for(size_t i{}; i < 64; ++i) {
            dest[i] = src[i] ^ round_constants_[n-1][63-i];
        }
...
//        std::cout << "K" << std::dec << n  << std::hex << "->";
//        println(dest);
    }

    void E(uint8_t* key, uint8_t* msg) {
    ...
    void g(uint8_t* msg) {
        ...
        uint8_t a[64] = {};
        a[1] = 2;
        ring_addition(a, N);//N += 512


    }

    void g(uint8_t* msg, size_t msg_length) {
        ring_addition(msg, checksum);
...
//        ring_addition(a, N);//N += 512
    ...
    void operator()(uint8_t* arr, size_t arr_length) {
        hash_array(arr, arr_length);
    }

    void operator()(uint8_t* arr, size_t arr_length, uint8_t* dest) {
        hash_array(arr, arr_length, dest);
    }
    ...
    void reset() {
        ...
        reset_array(N);
        reset_array(checksum);
    }

private:
    void hash(uint8_t* msg, size_t sz) {
        size_t msg_length = sz * 8;
        if(msg_length == 512) {
            g(msg);
        } else {
            uint8_t buf[64];
            size_t v = 0;
            std::fill(buf, buf+64, v);
            ...
            *(buf+i) = 0x01;
            ...
            uint8_t Nbuf[64] = {};
            uint8_t Sbuf[64] = {};
            std::copy(N, N+64, Nbuf);
            std::copy(checksum, checksum+64, Sbuf);
    ...
    void hash(uint8_t* msg, size_t sz, uint8_t* dest) {
        size_t msg_length = sz * 8;
        if(msg_length == 512) {
            g(msg);
        } else {
        ...
        std::copy(h, h+64, dest);
    }

    void reset_array(uint8_t* arr) {
        std::fill(arr, arr+64, 0);
    }
private:
    void hash_array(uint8_t* arr, size_t array_length) {
        uint8_t* cur_ptr = arr;

        size_t cur{};
        for(; cur < array_length; ) {
             if(cur+64 < array_length) {
                hash(cur_ptr, 64);
                cur += 64;
                cur_ptr += 64;
             } else {
                 hash(cur_ptr, array_length-cur);
                 break;
             }
        }
    }

    void hash_array(uint8_t* arr, size_t array_length, uint8_t* dest) {
        uint8_t* cur_ptr = arr;
...
uint8_t round_constants_[13][64] = {
        {},
...
//static
//uint8_t*
//hash_file(const std::string file_name) {
//
//    auto file_descriptor = std::fopen(file_name.data(), "rb");
//
//    Streebog hash{512};
//
//    uint8_t buf[64];
//    while(auto num = std::fread(buf, 1, 64, file_descriptor)) {
//
//    }
//
//
...
#endif //TESTPROJECT_STREEBOG_HPP